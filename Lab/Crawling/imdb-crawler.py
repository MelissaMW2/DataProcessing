...
def scrape_top_250(url):
    ...
    '''
    element = Element(dom)
    movies = element.get_element_by_classname('titleColumn')

    movie_urls = []
    for movie in movies
        # voor elke movie in movies => get URL
        url = movie.by_class[0].by_tag('a')[0].content
...
def scrape_movie_page(dom):
    ...
    for movie in movies
        title = movie.by_class('lister-item-header')[0].by_tag('a').content.encode('ascii','ignore')
